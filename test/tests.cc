...
void test_simple_input(void)
{
    ...
    token expected_tokens[9] = {
        {token_type::assign, '='},   {token_type::plus, '+'},
        {token_type::lparen, '('},   {token_type::rparen, ')'},
        {token_type::lsquirly, '{'}, {token_type::rsquirly, '}'},
    ...
    for (auto &expected : expected_tokens) {
        ...
        if (token.type != expected.type) {
            std::cerr << "Expected token type: "
                      << std::to_underlying(expected.type) << " got "
                      << std::to_underlying(token.type) << '\n';

            std::exit(1);
        }
        else if (token.literal != expected.literal) {
            std::cerr << "Expected token literal: "
                      << std::get<char>(expected.literal) << " got "
